...
const userResponses = [];
// Константы и переменные продуктов удалены
...
async function initializeApp() {
  ...
  // --- Шаг 2: Инициализация Telegram WebApp ---
  try {
  ...
  // --- Шаг 3: Инициализация для всех пользователей ---
  // Запускаем опрос для всех пользователей (включая Telegram)
  const userId = window.Telegram?.WebApp?.initDataUnsafe?.user?.id?.toString() || 'mock_user_12345';
...
async function loadMainContent(archetype) {
  ...
  // Продукты удалены - только квесты
...
// Функция populateProducts удалена

// Функция createProductCard удалена

// Функция initializeProductInteraction удалена

// Функции handlePurchase и onSuccessfulPurchase удалены

// Функции PDF viewer удалены
