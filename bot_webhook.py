...
async def handle_lava_payment(update: Update, context: CallbackContext):
    ...
    # Создаем инвойс напрямую через Lava Top API
    try:
        print("🔧 Создаем инвойс через Lava Top API...")
        ...
        if response.status_code == 200:
            ...
            # Получаем invoice_id из ответа
            invoice_id = result.get('data', {}).get('id')
            if invoice_id:
                print(f"✅ Инвойс создан с ID: {invoice_id}")
                
                # Формируем ссылку как в примере
                product_id = LAVA_PRODUCT_ID
                product_url_id = LAVA_PRODUCT_URL_ID
                
                # Создаем paymentParams (упрощенная версия)
                payment_params = {
                    ...
                    "paymentSettings": {
                        ...
                        "provider": {
                            "name": "smart_global",
                            "parameters": {
                                "public_token": "3a1a65b113077d853151f2126638d9bdcf2f3dfe02104b63c9c338d0624f2cfe",
                                "stylesheet": "https://widget.smart-global.com/payment-form.css",
                                "script": "https://widget.smart-global.com/payment-form.js"
                            }
                        },
                        "periodicity": "period_30"
                    }
                }
                
                # Кодируем paymentParams в base64
                import json
                import base64
                payment_params_json = json.dumps(payment_params, separators=(',', ':'))
                payment_params_encoded = base64.b64encode(payment_params_json.encode()).decode()
                
                # Формируем финальную ссылку
                payment_url = f"https://app.lava.top/products/{product_id}/{product_url_id}?paymentParams={payment_params_encoded}"
                
                print(f"🔗 Созданная ссылка: {payment_url}")
                ...
                print(f"❌ Invoice ID не найден в ответе: {result}")
        else:
