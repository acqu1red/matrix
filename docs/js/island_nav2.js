...
scene.add(sun);
scene.add(new THREE.HemisphereLight(0xbcd9ff, 0x22334a, 0.7));

const sandTex = new THREE.TextureLoader().load('assets/sand_4k.jpg'); sandTex.wrapS = sandTex.wrapT = THREE.RepeatWrapping; sandTex.repeat.set(6,6);
const ground = new THREE.Mesh(new THREE.CylinderGeometry(60,60,0.6,64), new THREE.MeshStandardMaterial({map:sandTex, roughness:.95, metalness:.02}));
...
const stoneTex = new THREE.TextureLoader().load('assets/temple_stone_4k.jpg'); stoneTex.wrapS=stoneTex.wrapT=THREE.RepeatWrapping; stoneTex.repeat.set(1,1);
const temples = new THREE.Group(); scene.add(temples);
const raysMat = new THREE.SpriteMaterial({ map: new THREE.TextureLoader().load('assets/glow.png'), color:0x36c2b6, transparent:true, opacity:0.0, depthWrite:false });
const templesArr = [];
function makeTemple(i){
  const g = new THREE.Group();
  const mat = new THREE.MeshStandardMaterial({ map: stoneTex, roughness:0.6, metalness:0.1, color: new THREE.Color().setHSL(0.53 + i*0.05, 0.25, 0.65) });
  ...
  const colMat = new THREE.MeshStandardMaterial({ map: stoneTex, roughness:0.5, color:0xffffff });
  for (let k=0;k<8;k++){
    const col = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.22, 1.8, 14), colMat);
    const ang = k*Math.PI/4;
    col.position.set(Math.cos(ang)*2.0, 1.3, Math.sin(ang)*2.0);
    col.castShadow = true; g.add(col);
  }
  const glow = new THREE.Sprite(raysMat.clone()); glow.scale.set(3.8,3.8,1); glow.position.y = roof.position.y + 0.8; g.add(glow);
...
const CLICK = {x:0,y:0,time:0}; let selected = null;
...
function onUp(e){
  ...
  if (dt<350 && dx<0.02 && dy<0.02){
    ...
    if (selected === g){
      ...
      const sy = ((e.touches?e.changedTouches[0].clientY:e.clientY)-rect.top)/rect.height;
