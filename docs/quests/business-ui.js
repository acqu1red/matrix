...
class BusinessQuestUI {
  ...
  setupEventListeners() {
    const myBizBtn = document.getElementById('btnMyBusiness');
    if (myBizBtn) { myBizBtn.addEventListener('click', () => this.openMyBusiness()); }
  ...
  // Выбор ниши по клику и авто-переход
  selectNiche(nicheId) {
    // Сброс выделения и выделение текущей
    ...
    // Сохранить выбор и перейти дальше
    if (this.engine.selectNiche(nicheId)) {
      const btn = this.elements.buttons && this.elements.buttons.confirmNiche;
