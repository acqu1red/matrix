// ГЛУБИННОЕ ГОСУДАРСТВО - АРХИВЫ ЭПШТЕЙНА
class WorldGovernmentStories {
  constructor() {
    this.storyIndex = 0;
    this.currentStory = null;
    this.epsteinLevel = 1; // Уровень заговора
    this.shadowNetwork = [];
    this.exposureRisk = 0; // Риск разоблачения
  }

  // АРХИВЫ ЭПШТЕЙНА - ВСТУПИТЕЛЬНЫЕ СЮЖЕТЫ
  generateIntroStories() {
    return [
      {
        title: "🔺 ОПЕРАЦИЯ 'ОСТРОВ ТЕНЕЙ' АКТИВИРОВАНА",
        content: "📝 **СЕКРЕТНЫЙ ОТЧЁТ - КОД ДОСТУПА: ULTRA**\n\nАрхивы Джеффри Эпштейна раскрыты. Чёрная книга с контактами мировой элиты, тайные записи встреч на острове, финансовые потоки через офшоры - всё это теперь в ваших руках.\n\n🎭 **КОМПРОМАТ НА КЛЮЧЕВЫХ ФИГУР:**\n• 47 политиков высшего уровня\n• 23 миллиардера и олигарха\n• 15 глав спецслужб разных стран\n• 31 медиамагнат и владелец техкорпораций\n\nНачинается игра на ставках судьбы человечества.",
        type: "intro"
      },
      {
  ...
  // КРУТЫЕ ЗАВЕРШАЮЩИЕ СЦЕНАРИИ В СТИЛЕ ЭПШТЕЙНА
  getEpsteinFinaleStory(sectors, failureProbability) {
    const totalAgents = Object.values(sectors).reduce((sum, sector) => sum + sector.members.length, 0);
    const dominantSector = this.getDominantSector(sectors);
    
    if (failureProbability <= 15) {
      return this.getEpsteinTriumphStory(totalAgents, dominantSector);
    } else if (failureProbability <= 40) {
      return this.getEpsteinCompromiseStory(totalAgents, dominantSector);
    } else if (failureProbability <= 70) {
      return this.getEpsteinExposureStory(totalAgents, dominantSector);
    } else {
      return this.getEpsteinDownfallStory(totalAgents, dominantSector);
    }
  }

  getEpsteinTriumphStory(totalAgents, dominantSector) {
    const stories = [
      {
        title: "🔺 НОВЫЙ МИРОВОЙ ПОРЯДОК УСТАНОВЛЕН",
        phases: [
          {
            title: "ФАЗА I: АКТИВАЦИЯ СЕТИ ЭПШТЕЙНА",
            content: `📋 **СЕКРЕТНЫЙ ОТЧЕТ - ОПЕРАЦИЯ "ВОСХОД ПИРАМИДЫ"**
            
🎯 **СТАТУС**: УСПЕШНО ЗАВЕРШЕНО
📅 **ДАТА**: ${new Date().toLocaleDateString('ru-RU')}

💀 Чёрная книга Джеффри Эпштейна была использована для создания послушной глобальной элиты.

🌐 **ГЛОБАЛЬНАЯ СЕТЬ АКТИВИРОВАНА:**
${dominantSector === 'political' ? '🏛️ ПОЛИТИЧЕСКАЯ МАШИНА ДОМИНИРУЕТ' : ''}
${dominantSector === 'economic' ? '💰 ФИНАНСОВАЯ ОЛИГАРХИЯ ПРАВИТ' : ''}
${dominantSector === 'military' ? '⚔️ ВОЕННЫЙ КОМПЛЕКС КОНТРОЛИРУЕТ' : ''}

⚡ **ОПЕРАЦИИ ПО ВСЕМУ МИРУ:**
• "Случайные" отставки 23 премьер-министров
• "Сердечные приступы" у 12 оппозиционных лидеров  
• "Технические сбои" на выборах в 34 странах

🎭 Мир танцует под вашу музыку, не зная дирижёра.`,
            rewards: { mulacoin: 2500, exp: 6000 }
          }
        ]
      }
    ];
  ...
  getEpsteinCompromiseStory(totalAgents, dominantSector) {
    return {
      title: "⚖️ ТАЙНАЯ СДЕЛКА С ДРУГИМИ ТЕНЯМИ",
      phases: [
        {
          title: "САММИТ КУКЛОВОДОВ",
          content: `🌍 **СЕКРЕТНЫЕ ПЕРЕГОВОРЫ В ШВЕЙЦАРСКИХ АЛЬПАХ**

🤵 **УЧАСТНИКИ ТЕНЕВОГО САММИТА:**
- Ваши представители: Архитектор-001
- Ротшильдский клан: Барон Н.
- Китайские триады: Господин Дракон

💼 **СОГЛАШЕНИЕ О РАЗДЕЛЕ МИРА:**
🌎 Америка - под вашим контролем (70%)
🌍 Европа - совместное управление (40%)
🌏 Азия - триады + вы (30%)

🤝 Мировая война кукловодов предотвращена... пока.`,
          rewards: { mulacoin: 1500, exp: 3000 }
        }
      ]
    };
  }

  getEpsteinExposureStory(totalAgents, dominantSector) {
    return {
      title: "🚨 ЧАСТИЧНАЯ УТЕЧКА - ОПЕРАЦИЯ ПОД УГРОЗОЙ",
      phases: [
        {
          title: "ЖУРНАЛИСТСКОЕ РАССЛЕДОВАНИЕ",
          content: `📰 **ЭКСТРЕННЫЕ НОВОСТИ - WIKILEAKS 2.0**

🔍 Группа хакеров "ГлубокийКролик" опубликовала часть архивов:

💾 **СКОМПРОМЕТИРОВАННЫЕ ДАННЫЕ:**
- 34% списков из чёрной книги Эпштейна
- Записи 67 секретных встреч
- Переводы на $2.3 млрд через офшоры

🌐 **РЕАКЦИЯ:**
• #EpsteinFiles - топ тренд 48 часов
• Протесты в 28 столицах
• Требования расследования

⚡ **ПЛАН КОНТРОЛЯ УЩЕРБА:**
🛡️ Дезинформационные боты активированы
💀 "Самоубийства" 5 свидетелей

🎲 Игра продолжается, ставки выросли.`,
          rewards: { mulacoin: 800, exp: 1500 }
        }
      ]
    };
  }

  getEpsteinDownfallStory(totalAgents, dominantSector) {
    return {
      title: "💥 ПОЛНОЕ РАЗОБЛАЧЕНИЕ - КРАХ ИМПЕРИИ",
      phases: [
        {
          title: "ОПЕРАЦИЯ ОБОРВАЛАСЬ",
          content: `🚨 **BREAKING NEWS - ГЛОБАЛЬНАЯ СЕНСАЦИЯ**

"Проект Пандора" раскрыл полные архивы сети Эпштейна.

💥 **ПОСЛЕДСТВИЯ:**
- 89 ордеров на арест в 34 странах
- Активы на $847 млрд заморожены
- Правительства 18 стран пали

📱 **ИНФОРМАЦИОННАЯ РЕВОЛЮЦИЯ:**
- #EpsteinConspiracy - топ хештег в истории
- Музеи "преступлений элиты" в 50 городах

💀 **СТАТУС СЕТИ:**
- 78% агентов арестованы
- Ваш статус: В МЕЖДУНАРОДНОМ РОЗЫСКЕ

🎭 *"Империи падают, но кукловоды возрождаются..."*`,
          rewards: { mulacoin: 100, exp: 300 }
        }
      ]
    };
  }

  getDominantSector(sectors) {
    return Object.entries(sectors)
      .sort(([,a], [,b]) => b.members.length - a.members.length)[0][0];
  }
}
