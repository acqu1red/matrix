...
def api_pay_create():
    ...
    if not res.get("ok"):
        return jsonify({"ok": False, "error": f"lava invoice error: {res.get('error')}"}), 502
...
def api_pay_hook():
    ...
    if success and tg_id:
        ...
        if invite:
            ...
            _send_telegram_message(int(tg_id), "✅ Оплата прошла успешно! Но нам не удалось создать инвайт автоматически, напишите администратору.")
    else:
